...
[My GitHub Repository](https://github.com/Lexie1xx1/Workshop9.2.git)
...
# ## When echo is TRUE, we can see the code and the result of running it. With echo equal to FALSE, we can only see the result of the code running, and the code will be hidden.But if it is pure text, it does not affect.
```
...
data_nn <- data_n %>% select("species","treatment","animal_id","activity")
data_nn
```
```{r}
# Check spelling in species and treatment but also generate a summary table
data_nnw <- data_nn %>% filter(species!="whitedams")
data_nnw
```
```{r}
# Use flextable to render the summary table in a tidy format
ft <- data_nnw %>% group_by(species,treatment) %>% summarise(mean=mean(activity),standard_error=sd(activity),sample_size=length(treatment))
flextable(ft)
```
...
## Red is what's been removed, green is what's been added.
```
...
ggplot(data_nn, mapping = aes(x = treatment, y = activity, colour= species)) + geom_boxplot() + facet_wrap(vars(species))
```
...
```{r,savefig}
library(png)
acantho <- readPNG("acantho.png")
ambon <- readPNG("ambon.png")
chromis <- readPNG("chromis.png")
humbug <- readPNG("humbug.png")
lemon <- readPNG("lemon.png")
whitedams <- readPNG("whitedams.png")
ggplot(data_nn, mapping = aes(x = species, y = activity, colour= treatment)) + geom_boxplot() + annotation_raster(acantho,0.5,1.5,50,60) + annotation_raster(ambon,1.5,2.5,50,60) + annotation_raster(chromis,2.5,3.5,50,60) + annotation_raster(humbug,3.5,4.5,50,60) + annotation_raster(lemon,4.5,5.5,50,60) + annotation_raster(whitedams,5.5,6.5,50,60)
```
...
After writing the image path in the TXT document, the image is no longer available on GitHub.
```
...
When uploaded to Github, they are visible to collaborators in the folder, even though they are PNG files.
```
...
Three <- data_nn %>% filter(data_nn$species==c("chromis","lemon","acantho"))
ggplot(Three, mapping = aes(x = species, y = activity, colour= treatment)) + geom_boxplot() + annotation_raster(chromis,0.5,1.5,40,60) + annotation_raster(lemon,1.5,2.5,40,60) + annotation_raster(acantho,2.5,3.5,40,60)
```
...
data_acantho <- data_nn %>% filter(species=="acantho")
t.test_acantho <- t.test(data_acantho$activity[which(data_acantho$treatment=="CO2")],data_acantho$activity[which(data_acantho$treatment=="control")])
t.test_acantho
```
```{r}
data_ambon <- data_nn %>% filter(species=="ambon")
t.test_ambon <- t.test(data_ambon$activity[which(data_ambon$treatment=="CO2")],data_ambon$activity[which(data_ambon$treatment=="control")])
t.test_ambon
```
```{r}
data_chromis <- data_nn %>% filter(species=="chromis")
t.test_chromis <- t.test(data_chromis$activity[which(data_chromis$treatment=="CO2")],data_chromis$activity[which(data_chromis$treatment=="control")])
t.test_chromis
```
```{r}
data_humbug <- data_nn %>% filter(species=="humbug")
t.test_humbug <- t.test(data_humbug$activity[which(data_humbug$treatment=="CO2")],data_humbug$activity[which(data_humbug$treatment=="control")])
t.test_humbug
```
```{r}
data_lemon <- data_nn %>% filter(species=="lemon")
t.test_lemon <- t.test(data_lemon$activity[which(data_lemon$treatment=="CO2")],data_lemon$activity[which(data_lemon$treatment=="control")])
t.test_lemon
```
```{r}
data_whitedams <- data_nn %>% filter(species=="whitedams")
t.test_whitedams <- t.test(data_whitedams$activity[which(data_whitedams$treatment=="CO2")],data_whitedams$activity[which(data_whitedams$treatment=="control")])
t.test_whitedams
```
...
p.value <- c(t.test_acantho$p.value, t.test_ambon$p.value, t.test_chromis$p.value, t.test_humbug$p.value, t.test_lemon$p.value, t.test_whitedams$p.value)
df <- c(t.test_acantho$parameter, t.test_ambon$parameter, t.test_chromis$parameter, t.test_humbug$parameter, t.test_lemon$parameter, t.test_whitedams$parameter)
t.statistic <- c(t.test_acantho$statistic, t.test_ambon$statistic, t.test_chromis$statistic, t.test_humbug$statistic, t.test_lemon$statistic, t.test_whitedams$statistic)
confi1 <- c(min(t.test_acantho$conf.int), min(t.test_ambon$conf.int), min(t.test_chromis$conf.int), min(t.test_humbug$conf.int), min(t.test_lemon$conf.int), min(t.test_whitedams$conf.int))
confi2<- c(max(t.test_acantho$conf.int), max(t.test_ambon$conf.int), max(t.test_chromis$conf.int), max(t.test_humbug$conf.int), max(t.test_lemon$conf.int), max(t.test_whitedams$conf.int))
difference.in.means <- c(abs(max(t.test_acantho$estimate)-min(t.test_acantho$estimate)), abs(max(t.test_ambon$estimate)-min(t.test_ambon$estimate)), abs(max(t.test_chromis$estimate)-min(t.test_chromis$estimate)), abs(max(t.test_humbug$estimate)-min(t.test_humbug$estimate)), abs(max(t.test_lemon$estimate)-min(t.test_lemon$estimate)), abs(max(t.test_whitedams$estimate)-min(t.test_whitedams$estimate)))
...
lemon_1 <- data_lemon
lemon_1$treatment <- lemon_1$treatment[sample(length(data_lemon$treatment))]
head(lemon_1)
...
for (i in 1:n_reps)
{
    lemon_11 <- data_lemon %>% mutate(treatment = treatment[sample(row_number())])
    # The results of t-test
    permuted_test <- t.test(lemon_11$activity[which(lemon_11$treatment=="CO2")],lemon_11$activity[which(lemon_11$treatment=="control")])
    samp_dist[i] <- permuted_test$statistic
}

t_t.test_lemon <- t.test_lemon$statistic
plot_tibble <- tibble(value = samp_dist,extreme = (abs(samp_dist) > abs(t_t.test_lemon)))
## Draw a histogram
ggplot(plot_tibble,aes(x=value,fill=extreme))+geom_histogram(bins=200)
```
```{r}
permutation_pvalue <- sum(plot_tibble$extreme)/n_reps
tibble(permutation_pvalue,t.test_lemon$p.value)
```
...
The difference between the two p-values is small, indicating that there is little difference between the results of lemon's control and CO2 treatments.
# **Task 10**
